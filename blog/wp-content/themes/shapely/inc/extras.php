...
function shapely_get_header_logo() {
	...
	if ( $custom_logo_id ) {
		 ...
		 */
		$html = sprintf( '<a href="%1$s" class="custom-logo-link" rel="home" itemprop="url">%2$s</a>', esc_url( 'https://adaptspace.com.br/' ), wp_get_attachment_image( $custom_logo_id, $dimension, false, $custom_logo_attr ) );
	} // If no logo is set but we're in the Customizer, leave a placeholder (needed for the live preview).
	elseif ( is_customize_preview() ) {
		$html = sprintf( '<a href="%1$s" class="custom-logo-link"><img class="custom-logo"/ style="display:none;"><span class="site-title">%2$s</span></a>', esc_url('https://adaptspace.com.br/' ), esc_html( get_bloginfo( 'name' ) ) );
	} else {
